...
void display_iringbuf() {
    ...
    for (i = 0; i < cur;i++) {
      ptr = buf;
      ptr += sprintf(buf, "%s %x: %08x     ", (i+1) % MAX_INST_TO_RECORD == cur ? "    -->" : "       ", iringbuf[i].pc, iringbuf[i].inst);
...
void mtrace_read(uint32_t addr, int len) {
  printf("Paddr read  at %x len = %d\n", addr, len);
}

void mtrace_write(uint32_t addr, int len, uint32_t data) {
  printf("Paddr write at %x len = %d data = %x\n", addr, len, data);
}
...
static ftrace getTables(Elf32_Ehdr *hdr, int fd) {
    ...
    sectNames = (char *) malloc(s.sh_size);
...
void insert_call_record(uint32_t pc, int depth){
...
int searchSymbol(uint32_t pc, bool check_return){
	for (int i = 0;i < FUNC_Num;i++) {
		if (check_return) {
...
void function_call(uint32_t pc, uint32_t dnpc){
...
void function_return(uint32_t pc){
