...
uint32_t get_operator(uint32_t p, uint32_t q){
  uint32_t ret = 0;
  uint32_t par = 0;
  uint32_t op_type = 0;
  uint32_t tmp_type = 0;
  ...
  for (uint32_t i = p;i <= q; i++){
    ...
    if (tokens[i].type == TK_NUMBER || tokens[i].type == TK_HEX || tokens[i].type == TK_REG) {   //数字都不是主运算符
      ...
      switch (tokens[i].type) {      //根据case对于不同符号赋值并对比得出主运算符
        ...
        default: printf("No operator detected"); assert(0);
      }
      //printf("%d\n", tmp_type);
      //printf("%d\n", op_type);
      if (tmp_type >= op_type) {
...
uint32_t eval(uint32_t p, uint32_t q) {
  if (p > q) {
    ...
    uint32_t op_type = get_operator(p, q);
    //Log("%d", op_type);
    if (tokens[op_type].type == TK_NEGATIVE || tokens[op_type].type == TK_POINTER_DEREF) {
      ...
      switch (tokens[op_type].type) {
        ...
        case TK_POINTER_DEREF: return pmem_read(val, 4);
      }
    } 
